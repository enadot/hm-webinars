      ...
      <div className="container relative mx-auto px-4">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold text-brand-dark mb-8 leading-[1.05] tracking-tight">
            ...
            הכללים השתנו.
            <br />
            <span className="text-gradient-gold">אתם משחקים נכון?</span>
          </h2>

          <p className="text-xl sm:text-2xl md:text-3xl text-brand-ink/80 leading-[1.5] mb-6 font-medium">
            ריביות גבוהות, מחירים תנודתיים ובנקים שמסרבים לאשר משכנתאות.
          </p>

          <p className="text-lg sm:text-xl md:text-2xl text-muted-foreground leading-[1.65] mb-14 max-w-4xl mx-auto">
            ההפרש בין החלטה מבוססת נתונים להחלטה רגשית - יכול להיות{" "}
            <strong className="text-brand-primary font-extrabold">עשרות אלפי שקלים</strong>.
            הוובינר הזה נותן לכם את הראייה הנכונה - של מומחה ההשקעות,
            וגם של מומחה המשכנתאות - כדי שתבצעו את המהלך הבא שלכם בביטחון.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-5 md:gap-6 max-w-4xl mx-auto">
            <FeatureBlock
              ...
              title="מודלים מוכחים"
              subtitle="לניהול סיכונים - לא ניחושים אלא מתודולוגיה"
              color="primary"
            />
            <FeatureBlock
              icon={<Lightbulb className="size-10" />}
              title="12+ שנות שטח"
              subtitle="כלים יישומיים שעובדים, לא תיאוריה"
              color="gold"
            />
          </div>
        </div>
...
function FeatureBlock({
  ...
  return (
    <div className="flex items-center gap-5 p-6 md:p-7 rounded-3xl bg-slate-50 border-2 border-slate-200/80 hover:border-brand-gold/40 hover:bg-white hover:-translate-y-1 transition-all duration-300 text-start shadow-sm hover:shadow-card-lift">
      <div
        className={`flex items-center justify-center size-20 rounded-2xl shrink-0 shadow-lg ${iconBg}`}
      >
        {icon}
      </div>
      <div>
        <div className="text-xl md:text-2xl font-extrabold text-brand-dark mb-1">
          {title}
        </div>
