...
export function ScAudience({ config }: { config: CampaignConfig }) {
  ...
  return (
    <EditableSection sectionKey="audience" className="bg-[#EFEFEF] text-[#162321] py-16 md:py-[104px]">
      <div className="max-w-[1160px] mx-auto px-5 md:px-6 box-border">
        ...
        <EditableText
          ...
          as="h2"
          multiline
          className="m-0 mb-8 md:mb-12 font-black text-[32px] md:text-[clamp(36px,5.4vw,72px)] leading-[1.1] md:leading-[1.07] tracking-[-0.8px] md:tracking-[-1.4px] max-w-[20ch] [text-wrap:balance]"
          placeholder="כותרת"
        />
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,290px),1fr))] gap-4">
          {cards.map((c, i) => (
            <blockquote
              key={i}
              className={`m-0 rounded-[20px] px-5 py-5 md:px-[26px] md:py-7 ${
                c.dark ? "bg-[#162321] text-[#EFEFEF]" : "bg-white border border-[#DCE3DF]"
              }`}
            >
              <span
                aria-hidden
                className={`block font-tae text-[36px] md:text-[44px] leading-[.6] ${
              ...
              <EditableText
                ...
                as="div"
                multiline
                className="font-extrabold text-[18px] md:text-[clamp(18px,1.9vw,23px)] leading-[1.45] mt-3 mb-2.5 md:mt-3.5 md:mb-3"
                placeholder="ציטוט"
              />
              <EditableText
        ...
        <EditableText
          ...
          as="p"
          multiline
          className="mt-8 md:mt-11 mx-auto mb-0 text-center max-w-[52ch] text-[17px] md:text-[clamp(17px,1.8vw,21px)] leading-[1.6] md:leading-[1.65] text-[#4A5C54] [text-wrap:pretty]"
