...
#### 9. ✅ מייל אישור אוטומטי + תזכורות (בוצע)
לאחר הרשמה נשלח מייל אישור דרך Resend (`RESEND_API_KEY` + `RESEND_FROM`).
תזכורות 24 שעות ושעה לפני נקבעות אוטומטית בשמירת קמפיין — **רק** כשמוגדר
`webinarJoinUrl`, ונשלחות כ-broadcast מתוזמן לרשימת שלח מסר של הקמפיין.
קישור הכניסה נשמר כעמודה על `Campaign` ולא ב-`config`, כדי שלא ידלוף לדף הציבורי.
